...
void UnitTest::print_header() {
	...
	std::cout << BOLD_CYAN << "│ " << std::left << std::setw(49) << get_header()
			  << "│" << RESET << '\n';
...
void UnitTest::collect_results() {
	...
	std::cout << BOLD_CYAN << "│ " << BOLD_GREEN << std::left << std::setw(49)
			  << get_passed_test_text() << BOLD_CYAN << " │" << RESET << '\n';

	if (_failed_count) {
		std::cout << BOLD_CYAN << "│ " << BOLD_RED << std::left << std::setw(49)
				  << get_failed_test_text() << BOLD_CYAN << " │" << RESET
				  << '\n';
	}
	std::cout << BOLD_CYAN << "└" << std::string(50, '=') << "┘" << RESET
			  << '\n';
